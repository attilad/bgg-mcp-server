...
async function fetchBggXml(endpoint: string, params: Record<string, string | number>) {
  try {
...
server.tool(
  ...
  async ({ query, exact }) => {
    try {
      ...
      const gameResults = data.items.item.map((item: any) => ({
        id: item._id,
...
server.tool(
  "get-hot-games",
  "Get the current hottest board games on BoardGameGeek",
  {},
  async () => {
    try {
...
server.tool(
  "get-user-plays",
  "Get a user's recent board game plays",
  {
    username: z.string().describe("The BoardGameGeek username"),
